...
bash_task = BashOperator(task_id="run_bash_echo",
...
task_customer_landing_to_processing = BashOperator(
    ...
    bash_command='aws s3 mv s3://snowflakedatapipeline2022/firehose/customers/landing/ s3://snowflakedatapipeline2022/firehose/customers/processing/{0}/ --recursive'.format(
        batch_id),
    dag=dag
)
...
task_customers_processing_to_processed = BashOperator(
    ...
    bash_command='aws s3 mv s3://snowflakedatapipeline2022/firehose/customers/processing/{0}/ s3://snowflakedatapipeline2022/firehose/customers/processed/{0}/ --recursive'.format(
        batch_id),
    dag=dag
)
...
task_orders_landing_to_processing = BashOperator(
    ...
    bash_command='aws s3 mv s3://snowflakedatapipeline2022/firehose/orders/landing/ s3://snowflakedatapipeline2022/firehose/orders/processing/{0}/ --recursive'.format(
        batch_id),
    dag=dag
)
...
task_orders_processing_to_processed = BashOperator(
    ...
    bash_command='aws s3 mv s3://snowflakedatapipeline2022/firehose/orders/processing/{0}/ s3://snowflakedatapipeline2022/firehose/orders/processed/{0}/ --recursive'.format(
        batch_id),
    dag=dag
)
...
[task_orders_landing_to_processing >> snowflake_orders_sql_str >> task_orders_processing_to_processed, task_customer_landing_to_processing >>
    snowflake_customers_sql_str >> task_customers_processing_to_processed] >> snowflake_order_customers_small_transformation >> post_task
